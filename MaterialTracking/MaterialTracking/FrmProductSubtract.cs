...
{
    public partial class FrmProductSubtract : Form
    {
        public FrmProductSubtract()
        {
            InitializeComponent();
        }
        ...
        private void FrmProductSubtract_Load(object sender, EventArgs e)
        {
        ...
        private void FrmProductSubtract_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (videoCaptureDevice != null)
            {
                if (videoCaptureDevice.IsRunning)
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            cmb_productName.SelectedItem = info;
            txt_productSum.Text = dataGridView1.Rows[secilen].Cells[2].Value.ToString();
        ...
        private void btn_productSubtract_Click(object sender, EventArgs e)
        {
            if (measure == true)
            {
                try
                {
                    if (float.Parse(txt_productSum.Text) < float.Parse(txt_newValue.Text))
                    {
                        MessageBox.Show("Kullanacağınız miktar mevcut olan miktardan fazla olamaz. Lütfen tekrar deneyiniz."
                        , "Uyarı Paneli", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    if (float.Parse(txt_productSum.Text) >= float.Parse(txt_newValue.Text))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@p2", (float.Parse(txt_newValue.Text)) * (-1));
                        ...
                        MessageBox.Show(cmb_productName.SelectedItem.ToString() + " üründen " + txt_newValue.Text + " " + cmb_measure.SelectedItem + " kullanıldı."
                           , "Kayıt Paneli", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        txt_value.Text = "";

                        SqlCommand cmd3 = new SqlCommand();
                        cmd3.Connection = con.Open();
                        cmd3.CommandText = "SELECT CAST(SUM(product_amount) as DECIMAL(15,3)) FROM productAdd " +
                            "WHERE productAdd.product_barcod = '"+txt_barcode.Text+"' GROUP BY productAdd.product_barcod";
                        SqlDataReader dr3 = cmd3.ExecuteReader();
                        while (dr3.Read())
                            txt_productSum.Text = dr3[0].ToString();
                        cmd3.Dispose();
                        con.Close();
                    }                  
                }
                ...
                {
                    MessageBox.Show("Ürün kullanılırken bir hata oluştu. Lütfen tekrar deneyiniz."
                        , "Uyarı Paneli", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            if (measure == false)
        ...
        private void cmb_measure_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmb_measure.SelectedIndex != -1 && cmb_measure.SelectedIndex != 0)
            {
                ...
                if (txt_value.Text == "")
                {
                    MessageBox.Show("Lütfen ilk kullanılacak miktarı giriniz."
        ...
        private void btn_productSubstractHistory_Click(object sender, EventArgs e)
        {
            FrmProductSubstractHistory form = new FrmProductSubstractHistory();
            form.Show();
            this.Hide();
            if (videoCaptureDevice != null)
            {
