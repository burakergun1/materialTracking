...
{
    public partial class FrmProductSubstractHistory : Form
    {
        public FrmProductSubstractHistory()
        {
            InitializeComponent();
        }
        ...
        void infoDataGrid()
        {
            try
            {
                ...
                cmd2.CommandText = "SELECT productRegister.product_barcod AS 'Barkod', product_name AS 'Ürün Adı', " +
                    "(CAST((product_amount) as DECIMAL(15,3)) * -1) AS 'Miktar (Gram/MiliLitre)', productAdd.product_date AS 'Tarih' " +
        ...
        private void FrmProductSubstractHistory_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (checkBox2.Checked == true && checkBox1.Checked == false)
                {
                    ...
                    cmd.CommandText = "SELECT productRegister.product_barcod AS 'Barkod', product_name AS 'Ürün Adı', " +
                        "(CAST((product_amount) as DECIMAL(15,3)) * -1) AS 'Miktar', productAdd.product_date AS 'Tarih' " +
                        "FROM productRegister " +
                        "INNER JOIN productAdd ON productRegister.product_barcod = productAdd.product_barcod " +
                        "WHERE product_amount != 0 AND product_amount < 0 AND productAdd.product_date " +
                        "BETWEEN @p1 AND @p2 " +
                        "ORDER BY productAdd.product_date DESC";
                ...
                if (checkBox1.Checked == true && checkBox2.Checked == false)
                {
                    ...
                    cmd.CommandText = "SELECT productRegister.product_barcod AS 'Barkod', product_name AS 'Ürün Adı', " +
                        ...
                        "FROM productRegister " +
                        "INNER JOIN productAdd ON productRegister.product_barcod = productAdd.product_barcod " +
                        "WHERE product_amount != 0 AND product_amount < 0 productRegister.product_name = @p1 AND productRegister.product_barcod = @p2 " +
                        "ORDER BY productAdd.product_date DESC";
                    ...
                    cmd.Dispose();
                    con.Close();
                    MessageBox.Show(cmb_productName.SelectedItem.ToString() + " adlı ürünün kullanılma geçmişi başarıyla gösterildi."
                  , "Bilgi Paneli", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                if (checkBox1.Checked == true && checkBox2.Checked == true)
                {
                    ...
                    cmd.CommandText = "SELECT productRegister.product_barcod AS 'Barkod', product_name AS 'Ürün Adı', " +
                        ...
                        "FROM productRegister " +
                        "INNER JOIN productAdd ON productRegister.product_barcod = productAdd.product_barcod " +
                        "WHERE product_amount != 0 AND product_amount < 0 AND productRegister.product_name = @p1 AND productRegister.product_barcod = @p2 " +
                    ...
                    cmd.Dispose();
                    con.Close();
                    MessageBox.Show(date1 + " ile " + date2 + " tarihleri arasındaki " + cmb_productName.SelectedItem.ToString() + " adlı ürünün kullanılma geçmişi başarıyla gösterildi."
                  , "Bilgi Paneli", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception)
        ...
        private void btn_productSubstractMenu_Click(object sender, EventArgs e)
        {
        ...
        private void FrmProductSubstractHistory_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (videoCaptureDevice != null)
            {
                if (videoCaptureDevice.IsRunning)
